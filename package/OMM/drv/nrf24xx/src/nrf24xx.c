...
static void print_message_info(nrf24xx_msg_union_t *msg_to_send)
{
	...
	{
		printf(" %d", msg_to_send->msg.addr_from[i]);
	}
	printf("\r\n");
	...
	{
		printf(" %d", msg_to_send->msg.msg_buffer[i]);
	}
	printf("\r\n");
...
static void nrf24_prepare_and_send(nrf24xx_t *nrf24, uint8_t *ownaddress, nrf24xx_msg_union_t *msg_to_send, uint8_t *buf_tx, uint8_t buf_size)
{
	...
	printf("bufsize: %d how_many: %d rest: %d\n", buf_size, how_many, rest);

	nrf24_encode_message(msg_to_send, ownaddress, how_many, NULL, 0, 0);
...
static void nrf24_prepare_and_receive(nrf24xx_t *nrf24, nrf24xx_msg_union_t *msg_to_send, uint8_t *buf_rx, uint8_t buf_size)
{
	uint8_t j = 0;
	uint8_t how_many = 0;
...
int8_t nrf24_master_ping_pong(nrf24xx_t *nrf24, uint8_t *txaddress, uint8_t *rxaddress, uint8_t *buf_tx, uint8_t *buf_rx, uint8_t buf_size, uint8_t rx_req)
{
	...
	else
	{
		...
		nrf24_prepare_and_receive(nrf24, &msg_to_send, buf_rx, buf_size);
	}
